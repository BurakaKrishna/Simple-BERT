...
# init_checkpoint = model_dir + 'mrda-18'
...
with tf.Session() as sess:
    ...
    if training:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for train_step in range(1, num_training_steps + 1):
                ...
                # Evaluate every 'num_evaluations' training steps
                if train_step % (num_training_steps / num_evaluations) == 0:
                    ...
                    for eval_step in range(num_evaluation_steps):
                        ...
                        _, eval_batch_loss, eval_batch_logits, eval_batch_accuracy, eval_batch_predictions, eval_batch_labels = \
                            sess.run([optimizer, loss, logits, accuracy, predictions, label_ids],
    ...
    if testing:
        ...
        for test_step in range(1, num_test_steps + 1):
            _, test_batch_loss, test_batch_logits, test_batch_accuracy = \
                sess.run([optimizer, loss, logits, accuracy], feed_dict={handle_pl: test_handle})
            test_loss.append(test_batch_loss)
            test_accuracy.append(test_batch_accuracy)

        # Record training summaries
        test_summary = sess.run(summary,
                                    feed_dict={accuracy_pl: sum(test_accuracy) / len(test_accuracy),
                                                loss_pl: sum(test_loss) / len(test_loss)})
        test_writer.add_summary(test_summary, num_global_steps)

        # Display test statistics
        print("Testing loss: {:.3f}, acc: {:.2f}%".format((sum(test_loss) / len(test_loss)),
